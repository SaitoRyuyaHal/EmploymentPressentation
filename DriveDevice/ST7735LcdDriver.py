...
class ST7735(object):
    ...
    def __init__(self, dc, spi, rst=None, gpio=None, auxiliary=True, width=ST7735_TFTWIDTH, 
        ...
        self._gpio.setGpio()
        self._gpio.setMode(dc, GpioDriver.OUTPUT)
        # Setup reset as output (if provided).
        if rst is not None:
            self._gpio.setMode(rst, GpioDriver.OUTPUT)
        # Set SPI to mode 0, MSB first.
        if self.auxiliary is True:
            self._gpio.setSpi(spi, 12500000, 256)
        else:
            self._gpio.setSpi(spi, 12500000, 0)
        # Create an image buffer.
        self.buffer = Image.new('RGB', (width, height))

    def close(self):
        self._gpio.closeSpi(self._spi)

    def send(self, data, is_data=1, chunk_size=4096):
...
#        self._gpio.output(self._dc, is_data)
        self._gpio.digitalWrite(self._dc, is_data)
...
#            self._spi.write(data[start:end])
            self._gpio.spiDataWrite(self._spi, data[start:end])
...
#            self._gpio.set_high(self._rst)
            self._gpio.digitalWrite(self._rst, 1)
            time.sleep(0.500)
#            self._gpio.set_low(self._rst)
            self._gpio.digitalWrite(self._rst, 0)
            time.sleep(0.500)
#            self._gpio.set_high(self._rst)
            self._gpio.digitalWrite(self._rst, 1)
            time.sleep(0.500)
    ...
    def target_display(self, image=None, x0 = 0, y0 = 0, x1 = None, y1 = None):
        if image is None:
            image = self.buffer
        # Set address bounds to entire display.
...
def target_draw_rotated_text(text, angle, font, fill=(255,255,255)):
    # Get rendered font width and height.
    draw = ImageDraw.Draw(image)
    width, height = draw.textsize(text, font=font)
    # Create a new image with transparent background to store the text.
...
if __name__ == "__main__":
    ...
    RST = 26
    SPI_PORT = 1
    aulxiliary_flag = True
    disp = ST7735(DC, SPI_PORT, rst=RST, auxiliary=aulxiliary_flag)
    disp.begin()
    ...
    print('Loading image...')
    image = Image.open('./Python_ST7735/examples/cat.jpg')
    ...
    try:
        value = 0
        draw_rotated_text(disp.buffer, 'Hello World!', (50, 20), 90, font, fill=(255,255,255))
        disp.display()
        while(True):
            print('Drawing image')
            start_time = time.time()
            image = target_draw_rotated_text(str(value), 90, font, fill=(255, 255, 255))
            disp.target_display(image, 30, 20, 30+image.size[0]-1, 20+image.size[1]-1)
            end_time = time.time()
